/* ============================================================
   ...
   Perbaikan performa, klik, input, dan fitur baru.
   ============================================================ */
...
(function () {
    ...
    let chartOfAccounts = [];
    let budgets = [];
    let lastSalesCache = null;       // cache penjualan agar tidak dibaca ulang berkali‑kali
    let lastExpensesCache = null;
    ...
    // ==================== CACHE ELEMEN (hanya yang ada di HTML) ====================
    let E = {};
    ...
    function getSalesCached() {
        if (!lastSalesCache) lastSalesCache = Storage.getSales();
        return lastSalesCache;
    }
    function getExpensesCached() {
        if (!lastExpensesCache) lastExpensesCache = Storage.getExpenses();
        return lastExpensesCache;
    }
    function invalidateCache() {
        lastSalesCache = null;
        lastExpensesCache = null;
    }
    ...
    function setupSubTabs() {
        ...
        E.subTabBtns.forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                currentSubTab = tab;
                switch (tab) {
                    case 'finance-overview': refreshFinanceOverview(); break;
                    case 'finance-expense': renderRecentExpenses(); break;
    ...
    function getPeriod() {
        return {
            start: E.financePeriodStart?.value || getMonthStart(),
            end: E.financePeriodEnd?.value || getToday()
        };
    }
    ...
    async function refreshFinanceOverview() {
        ...
        const sales = getSalesCached().filter(s => s.tanggal >= start && s.tanggal <= end);
        const expenses = getExpensesCached().filter(e => e.tanggal >= start && e.tanggal <= end);
        const other = await loadOtherIncome();
        const otherIncomeTotal = other.filter(o => o.tanggal >= start && o.tanggal <= end).reduce((a, o) => a + o.jumlah, 0);
        ...
        // Chart berat, render hanya jika tab overview aktif
        if (currentSubTab === 'finance-overview') {
    ...
    // ==================== GRAFIK (dengan pengecekan apakah canvas ada & tab aktif) ====================
    function renderMonthlyPLChart() {
        if (!E.chartFinanceMonthlyPL || currentSubTab !== 'finance-overview') return;
    ...
    function renderExpenseCompositionChart(expenses) {
        if (!E.chartExpenseComposition || currentSubTab !== 'finance-overview') return;
    ...
    function renderMarginPerProductChart(sales) {
        if (!E.chartMarginPerProduct || currentSubTab !== 'finance-overview') return;
        const ctx = E.chartMarginPerProduct.getContext('2d');
        const map = {};
        sales.forEach(s => {
            if (!map[s.produk]) map[s.produk] = { rev: 0, hpp: 0 };
            map[s.produk].rev += (s.qty * s.hargaJual - (s.diskon || 0));
        ...
        marginPerProductChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Margin',
                    data: entries.map(([,d]) => d.rev - d.hpp),
                    backgroundColor: entries.map(([,d]) => (d.rev - d.hpp) < 0 ? '#ef4444' : '#22c55e'),
                    borderRadius: 4
                }]
            },
            options: {
    ...
    function renderRecentExpenses() {
        ...
        let expenses = getExpensesCached();
        ...
        E.recentExpensesList.innerHTML = recent.length === 0
            ...
            : recent.map(e => `<div class="flex justify-between py-2 border-b text-sm"><div><span class="font-medium">${e.akun}</span> <span class="text-xs opacity-70">${e.tanggal}</span>${e.deskripsi ? `<p class="text-xs opacity-50">${e.deskripsi}</p>` : ''}</div><span class="font-semibold text-red-500">${formatRupiah(e.jumlah)}</span></div>`).join('');
    }
    ...
    function buildJournalEntries(startDate, endDate) {
        const sales = getSalesCached().filter(s => s.tanggal >= startDate && s.tanggal <= endDate);
        const expenses = getExpensesCached().filter(e => e.tanggal >= startDate && e.tanggal <= endDate);
        const entries = [];
        sales.forEach(s => {
    ...
    function renderJournal(resetPage = true) {
        ...
        E.journalTableBody.innerHTML = pageEntries.length === 0
            ...
            : pageEntries.map(e => `<tr class="border-t text-sm"><td class="p-2">${e.tanggal}</td><td class="p-2">${e.deskripsi}</td><td class="p-2">${e.akun}</td><td class="p-2 text-right">${e.debet>0?formatRupiah(e.debet):''}</td><td class="p-2 text-right">${e.kredit>0?formatRupiah(e.kredit):''}</td></tr>`).join('');
    ...
    function refreshCashflow() {
        ...
        if (currentSubTab === 'finance-cashflow') renderCashflowChart(sales, expenses, startDate, endDate);
    }

    function renderCashflowChart(sales, expenses, startDate, endDate) {
        if (!E.chartCashflow) return;
    ...
    function refreshAnalysis() {
        ...
        const marginKotor = pendapatan ? ((labaKotor/pendapatan)*100).toFixed(1) : 0;
        const marginBersih = pendapatan ? ((labaBersih/pendapatan)*100).toFixed(1) : 0;
        const rasioBeban = pendapatan ? ((beban/pendapatan)*100).toFixed(1) : 0;
        const totalAset = Storage.getBatches().reduce((sum,b) => sum + ((b.berat - b.used) * (b.hargaBeli||0)), 0);
        const roa = totalAset ? ((labaBersih/totalAset)*100).toFixed(1) : 0;
        const ekuitas = totalAset + labaBersih;
        const roe = ekuitas ? ((labaBersih/ekuitas)*100).toFixed(1) : 0;
        ...
        if (E.analysisRecommendations) {
            let recs = [];
            if (marginBersih < 10) recs.push('Margin bersih rendah (<10%).');
            if (rasioBeban > 40) recs.push('Rasio beban tinggi (>40%).');
            if (roa < 5) recs.push('ROA rendah (<5%).');
            if (!recs.length) recs.push('Kinerja keuangan baik.');
    ...
    async function exportTaxPDF() {
        ...
        doc.save(`pajak_${getToday()}.pdf`);
        showToast('Sukses', 'PDF Pajak diunduh.', 'success');
    }

    async function checkTaxDueNotifications() {
        const settings = Storage.getSettings();
        const dueDate = settings.taxDueDate || 15;
        const today = new Date();
        const due = new Date(today.getFullYear(), today.getMonth(), dueDate);
        const daysLeft = Math.ceil((due - today) / 86400000);
        if (daysLeft <= 7 && daysLeft >= 0) {
            if (E.taxDueNotification) {
                E.taxDueNotification.innerHTML = `⚠️ Jatuh tempo pembayaran pajak bulan ini: ${due.toLocaleDateString('id-ID')} (${daysLeft} hari lagi)`;
    ...
    function renderCOA() {
        ...
        E.coaTableBody.innerHTML = chartOfAccounts.map(c => `<tr><td class="p-2">${c.kode}</td><td class="p-2">${c.nama}</td><td class="p-2">${c.tipe}</td><td class="p-2 text-center"><button class="btn btn-xs btn-danger" onclick="CFS.Accounting.deleteCoA('${c.kode}')">🗑️</button></td></tr>`).join('');
    }
    ...
    function renderBudget() {
        ...
        const rows = budgets.map(b => {
            ...
            return `<tr><td class="p-2">${akun ? akun.nama : b.kodeAkun}</td><td class="p-2">${b.bulan}</td><td class="p-2 text-right">${formatRupiah(b.jumlah)}</td><td class="p-2 text-center"><button class="btn btn-xs btn-danger" onclick="CFS.Accounting.deleteBudget('${b.id}')">🗑️</button></td></tr>`;
        }).join('');
        E.budgetTableBody.innerHTML = rows || '<tr><td colspan="4" class="text-center p-4 opacity-50">Belum ada anggaran.</td></tr>';
        if (currentSubTab === 'finance-budget') renderBudgetVsActualChart();
    }
    ...
    function renderBudgetVsActualChart() {
        if (!E.chartBudgetVsActual) return;
        const ctx = E.chartBudgetVsActual.getContext('2d');
        const { start, end } = getPeriod();
        const actualMap = {};
        getExpensesCached().filter(e => e.tanggal >= start && e.tanggal <= end).forEach(e => { actualMap[e.akun] = (actualMap[e.akun]||0) + e.jumlah; });
        const budgetMap = {};
        ...
        const labels = allAkun.map(k => { const a = chartOfAccounts.find(c => c.kode === k); return a ? a.nama : k; });
    ...
    async function refreshProjection() {
        ...
        for (let i = 11; i >= 0; i--) {
            ...
            last12.push(sales.filter(s => s.tanggal >= ms && s.tanggal <= me).reduce((sum,s) => sum + (s.qty * s.hargaJual - (s.diskon||0)), 0));
        }
        const n = last12.length;
        const x = Array.from({length:n}, (_,i)=>i);
        const sumX = x.reduce((a,b)=>a+b,0);
        const sumY = last12.reduce((a,b)=>a+b,0);
        const sumXY = x.reduce((s,xi,i) => s + xi*last12[i], 0);
        const sumX2 = x.reduce((s,xi) => s + xi*xi, 0);
        const slope = (n*sumXY - sumX*sumY) / (n*sumX2 - sumX*sumX);
        const intercept = (sumY - slope*sumX) / n;
        const next = Math.round(slope * n + intercept);
        if (E.projectionRevenue) E.projectionRevenue.textContent = formatRupiah(Math.max(0, next));
        if (E.projectionProfit) E.projectionProfit.textContent = formatRupiah(Math.max(0, Math.round(next * 0.3)));
        if (currentSubTab === 'finance-projection' && E.chartProjection) {
            const ctx = E.chartProjection.getContext('2d');
            if (projectionChart) projectionChart.destroy();
            projectionChart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: [...x.map(i => `Bulan -${12-i}`), 'Proyeksi'],
                    datasets: [{
                        label: 'Pendapatan', data: [...last12, next],
    ...
    async function refreshRatios() {
        ...
        const inventoryTurnover = totalAset ? (hpp / totalAset).toFixed(2) : 0;
        const netMargin = pendapatan ? ((labaBersih/pendapatan)*100).toFixed(1) : 0;
        const roa = totalAset ? ((labaBersih/totalAset)*100).toFixed(1) : 0;
        const roe = totalAset ? ((labaBersih/(totalAset)) * 100).toFixed(1) : 0;
    ...
    // ==================== EVENTS ====================
    function bindEvents() {
        if (E.applyFinancePeriod) E.applyFinancePeriod.addEventListener('click', () => { invalidateCache(); refreshFinanceOverview(); });
