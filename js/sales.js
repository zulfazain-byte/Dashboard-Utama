...
(function () {
    ...
    let globalDiscount = { type: 'nominal', value: 0 };
    let heldTransactions = [];        // disimpan di localforage
    let loyaltyInfo = { points: 0, redeem: 0 };
    ...
    async function initProductSearchDropdown() {
        ...
        // Barcode scanner support (dengarkan input keyboard cepat)
        let barcodeBuffer = '';
        let barcodeTimer;
    ...
    function getSelectedProduct() {
        return productDropdown ? productDropdown.getValue() : '';
    }
    ...
    function updateTotalDisplay() {
        ...
        E.itemsTotalDisplay.innerHTML = `
            <div class="text-sm space-y-1">
                ...
                <div>Diskon Item: ${formatRupiah(subtotal - totalAfterItemDiscount)}</div>
                <div>Diskon Global: -${formatRupiah(globalDisc)}</div>
                <div>Loyalty Redeem: -${formatRupiah(redeem)}</div>
                <div class="text-lg font-bold">Grand Total: ${formatRupiah(grandTotal)}</div>
            </div>`;
    }
    ...
    function renderPayments() {
        ...
        html += `<button class="btn btn-xs btn-secondary" onclick="CFS.Sales.addPaymentMethod()">+ Tambah Pembayaran</button>`;
        E.paymentsContainer.innerHTML = html;
    ...
    function getGrandTotal() {
        ...
        let globalDisc = 0;
        if (globalDiscount.type === 'percentage') {
            globalDisc = totalAfterItem * (globalDiscount.value / 100);
        } else {
            globalDisc = globalDiscount.value;
        }
    ...
    function openRecallModal() {
        ...
        E.heldTransactionsList.innerHTML = heldTransactions.map(t => {
            ...
            return `<div class="p-2 border rounded mb-2 flex justify-between">
                <div><strong>${t.klien || '-'}</strong> <span class="text-xs">${itemsDesc}</span></div>
                <div>
                    <span class="font-bold">${formatRupiah(total)}</span>
                    <button class="btn btn-xs btn-primary ml-2" onclick="CFS.Sales.recallTransaction('${t.id}')">Panggil</button>
                    <button class="btn btn-xs btn-danger" onclick="CFS.Sales.deleteHeldTransaction('${t.id}')">Hapus</button>
                </div>
            </div>`;
        }).join('') || '<p class="opacity-50">Tidak ada transaksi ditahan.</p>';
        E.recallModal.classList.remove('hidden');
    }
    ...
    function updateCustomerDisplay() {
        ...
        E.customerDisplayPanel.innerHTML = `
            <div class="p-4 bg-gray-900 text-white rounded-lg">
                <h3 class="text-lg font-bold mb-2">🛒 ${E.salesKlien?.value || 'Pelanggan'}</h3>
                <div class="text-sm">${itemsHtml || '<p>-</p>'}</div>
                <hr class="my-2">
                <div class="flex justify-between font-bold text-lg">Total: <span>${formatRupiah(total)}</span></div>
                <div class="text-sm">Pembayaran: ${formatRupiah(totalPaid)}</div>
                <div class="text-lg font-bold">Kembalian: ${formatRupiah(Math.max(0, change))}</div>
            </div>`;
    }
    ...
    async function processSale(e) {
        ...
        for (let item of items) {
            const reCalc = calculatePrice(item.produk, item.qty, tier, channel, item.hargaJual); // hargaJual sebagai acuan
            if (reCalc.error) {
    ...
    function printInvoice() {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text(`No. Invoice: INV-${currentTransactionId.toUpperCase()}`, 15, y);
        doc.text(`Tanggal: ${formatDate(first.tanggal)}`, 15, y + 6);
        doc.text(`Pelanggan: ${first.klien}`, 15, y + 12);
        y += 20;
        ...
        doc.text('Qty', 80, y);
        doc.text('Harga/kg', 100, y);
        doc.text('Diskon', 125, y);
        ...
        transSales.forEach(s => {
            ...
            doc.text(s.produk, 17, y);
            doc.text(`${s.qty} kg`, 80, y);
            doc.text(formatRupiah(s.hargaJual), 100, y);
            doc.text(formatRupiah(diskon), 125, y);
            doc.text(formatRupiah(subtotal - diskon), pageWidth - 15, y, { align: 'right' });
            y += 6;
        });
        ...
        doc.text(formatRupiah(grandTotal), pageWidth - 15, y, { align: 'right' });
        ...
        doc.text(`DPP: ${formatRupiah(dpp)}`, 100, y);
        doc.text(`PPN (${ppnRate}%): ${formatRupiah(ppn)}`, 100, y + 4);

        y = doc.internal.pageSize.getHeight() - 20;
        doc.setFontSize(8);
        doc.setTextColor(150);
        doc.text('Terima kasih atas kepercayaan Anda.', pageWidth / 2, y, { align: 'center' });
        doc.text(`Dicetak oleh ${company.name || 'Cibitung Frozen'} ERP Ultimate v5.4`, pageWidth / 2, y + 4, { align: 'center' });
    ...
    async function processRefund() {
        ...
        // Kembalikan stok ke batch (FEFO terbalik agar yang paling lama expired diisi dulu)
        const batches = Storage.getBatches().filter(b => b.produk === original.produk);
        if (batches.length) {
            batches.sort((a, b) => new Date(b.tglKadaluarsa) - new Date(a.tglKadaluarsa));
        ...
        E.todaySalesList.innerHTML = groups.map(group => {
            ...
            return `<div class="p-3 border rounded mb-2">
                <div class="flex justify-between">
                    <strong>${first.klien}</strong>
                    <span>${formatRupiah(total)}</span>
                </div>
                <div class="text-xs">${group.items.map(s => s.produk).join(', ')}</div>
                <div class="text-xs mt-1">
                    <button class="btn btn-xs btn-warning" onclick="CFS.Sales.openRefundModal('${group.transactionId}')">Retur</button>
                </div>
            </div>`;
        }).join('');
    }
    ...
    function renderHistory() {
        ...
        E.salesHistoryTableBody.innerHTML = groups.map(group => {
            ...
            return `<tr class="border-t text-sm">
                ...
                <td class="p-2 text-center">
                    <button class="btn btn-xs btn-danger" onclick="CFS.Sales.deleteTransaction('${group.transactionId}')">Hapus</button>
                    <button class="btn btn-xs btn-warning" onclick="CFS.Sales.openRefundModal('${group.transactionId}')">Retur</button>
                </td>
            </tr>`;
        }).join('');
    ...
    function shareWhatsApp() {
        ...
        const first = transSales[0];
        const grandTotal = transSales.reduce((sum, s) => sum + (s.qty * s.hargaJual - (s.diskon || 0)), 0);
        const itemsText = transSales.map(s => `- ${s.produk}: ${s.qty} kg x ${formatRupiah(s.hargaJual)}`).join('\n');
        const message = encodeURIComponent(
            `*INVOICE PENJUALAN*\n` +
            `🏪 *${company.name}*\n` +
            `📅 ${formatDate(first.tanggal)}\n` +
            `👤 ${first.klien}\n` +
            `📦 ${itemsText}\n` +
    ...
    function bindEvents() {
        ...
        if (E.cancelEditItemBtn) E.cancelEditItemBtn.addEventListener('click', resetItemForm);
